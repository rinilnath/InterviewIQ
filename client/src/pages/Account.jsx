...
import {
  ...
  Eye, EyeOff, Lock, Trash2, UserPlus, Mail, RotateCcw, Ban,
} from 'lucide-react';
...
function InviteSection({ currentUser }) {
  const queryClient = useQueryClient();
  ...
  const sendMutation = useMutation({
    mutationFn: () => api.post('/auth/invite', { email: inviteEmail.trim(), name: inviteName.trim() || undefined }),
    onSuccess: () => {
      queryClient.invalidateQueries(['invites']);
      setSendDialog(false);
      setInviteEmail('');
      setInviteName('');
      toast.success('Invite sent', `Invite sent to ${inviteEmail.trim()}`);
    },
    onError: (err) => toast.error('Failed', err.response?.data?.error || 'Could not send invite.'),
  });

  const revokeMutation = useMutation({
    mutationFn: (id) => api.delete(`/invites/${id}`),
    onSuccess: () => { queryClient.invalidateQueries(['invites']); toast.success('Revoked', 'Invite has been revoked.'); },
  ...
  const resendMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['invites']); toast.success('Resent', 'Invite email has been resent.'); },
    onError:   (err) => toast.error('Failed', err.response?.data?.error || 'Could not resend invite.'),
  });

  const invites = data?.invites || [];

  const canActOn = (invite) =>
    invite.invited_by === currentUser?.id || currentUser?.role === 'admin';

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold text-zinc-900">Invite Colleagues</h3>
          <p className="text-xs text-zinc-400 mt-0.5">Invite people to join InterviewIQ.</p>
        </div>
        <Button
          size="sm"
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {invites.map((inv) => {
                  ...
                  const canResend = (inv.status === 'pending' || inv.status === 'expired') && canActOn(inv);
                  const canRevoke = inv.status === 'pending' && canActOn(inv);
                  return (
          ...
          <div className="bg-zinc-900 px-5 py-4">
            <DialogHeader>
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] text-zinc-500 uppercase tracking-widest">Invite</span>
              </div>
              <DialogTitle className="text-white text-base font-semibold">Send an Invitation</DialogTitle>
              <DialogDescription className="text-zinc-400 text-xs mt-1">
                They'll receive a link to create their account (valid 48 hours).
              </DialogDescription>
            </DialogHeader>
          </div>
          <div className="bg-white px-5 py-5 space-y-4">
...
export default function Account() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      {/* Invite Colleagues */}
      <InviteSection currentUser={user} />
