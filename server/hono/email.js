...
const appUrl = (env) => (env.APP_URL || 'https://interviewiq-n403.onrender.com').replace(/\/$/, '');

async function logEmail(env, { emailType, recipientEmail, recipientName, subject, userId }) {
...
async function markLogFailed(env, logId, errorMessage) {
  if (!logId) return;
  const supabase = getSupabase(env);
  await supabase
    .from('email_logs')
...
function emailShell({ preheader = '', body, trackingId }, env) {
  const pixel = trackingId
    ? `<img src="${appUrl(env)}/api/email/track/${trackingId}" width="1" height="1" alt="" style="display:none;width:1px;height:1px" />`
    : '';
  return `<!DOCTYPE html>
<html lang="en">
...
export async function sendWelcomeEmail(env, { name, email, password, userId }) {
  const subject = "You've been granted access to InterviewIQ";
  const logId = await logEmail(env, { emailType: 'welcome', recipientEmail: email, recipientName: name, subject, userId });
  const loginUrl = `${appUrl(env)}/login`;
  ...
  try {
    ...
    await markLogFailed(env, logId, err.message);
  }
}

export async function sendSupportEmail(env, { fromName, fromEmail, fromTier, fromUserId, subject, message }) {
  const supportEmail = env.SUPPORT_EMAIL;
  const logId = await logEmail(env, { emailType: 'support', recipientEmail: supportEmail || 'unconfigured', recipientName: 'Admin', subject, userId: fromUserId });

  if (!supportEmail) {
    const msg = 'SUPPORT_EMAIL not set';
    console.warn('[Email] Support ticket not delivered —', msg, '| From:', fromEmail, '| Subject:', subject);
    await markLogFailed(env, logId, msg);
    return false;
  }
...
export async function sendInviteEmail(env, { toEmail, toName, fromName, token }) {
  const subject = "You've been invited to InterviewIQ";
  const logId = await logEmail(env, { emailType: 'invite', recipientEmail: toEmail, recipientName: toName || toEmail, subject });
  const registerUrl = `${appUrl(env)}/register?token=${token}`;
